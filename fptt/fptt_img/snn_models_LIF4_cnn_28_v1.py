...
class SNN(nn.Module):
    def __init__(self, input_size, hidden_size,output_size, n_timesteps, P=10):
        ...
        self.conv4 = SNN_Conv_cell(self.conv3.compute_output_size(),512,3,1,1,'max')

        f_shape = self.conv4.compute_output_size()
        self.snn1 = SNN_rec_cell(f_shape[0]*f_shape[1]*f_shape[2] , hidden_size,is_rec=False)
        self.snn2 = SNN_rec_cell(hidden_size, hidden_size)
        ...
        self.layers = [self.conv1,self.conv2,self.conv3,self.conv4,self.snn1,self.snn2]
    ...
    def forward(self, inputs, h):
        ...
        layer_i +=1
        mem_conv4,spk_conv4,b_conv4 = self.conv4(spk_conv3,h[layer_i*3],h[1+layer_i*3],h[2+layer_i*3])

        f_spike = torch.flatten(spk_conv4,1)
        # print(f_spike.shape)
        layer_i +=1
        ...
        for spk in [spk_conv1,spk_conv2,spk_1,spk_2]:
            self.fr = self.fr+ spk.detach().cpu().numpy().mean()/6.
