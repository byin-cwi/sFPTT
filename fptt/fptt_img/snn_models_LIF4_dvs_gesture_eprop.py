...
class SNN_rec_alif(nn.Module):
    def __init__(self, input_size, hidden_size,is_rec = True):
        super(SNN_rec_alif, self).__init__()
        # print('SNN-ltc ')
        ...
        if is_rec:
            ...
            self.layer1_x = nn.Linear(input_size, hidden_size)
        self.tau_adp = nn.Parameter(torch.Tensor(hidden_size))#,requires_grad=False)
        self.tau_m =nn.Parameter(torch.Tensor(hidden_size))#,requires_grad=False)
        self.act1 = nn.Sigmoid()

        # nn.init.normal_(self.tau_adp, 5,1)
        # nn.init.normal_(self.tau_m, 5.,1)
        # nn.init.normal_(self.tau_adp, 200,50)
        # nn.init.normal_(self.tau_m, 20,5)
        nn.init.normal_(self.tau_adp, 200,.1)
        nn.init.normal_(self.tau_m, 20,.05)
...
class SNN(nn.Module):
    def __init__(self, input_size, hidden_size,output_size, n_timesteps, P=10):
        ...
        self.snn3 = SNN_rec_alif(2048,hidden_size)
