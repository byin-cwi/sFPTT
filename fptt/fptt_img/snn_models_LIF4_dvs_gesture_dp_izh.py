...
def save_checkpoint(state, is_best, prefix, filename='_snnl4s2-izh_checkpoint.pth.tar'):
    print('saving at ', prefix+filename)
    torch.save(state, prefix+filename)
    if is_best:
        shutil.copyfile(prefix+filename, prefix+ '_snnl4s2-izh_model_best.pth.tar')
...
def mem_update_adp(inputs, mem, spike, tau_adp,tau_m, b, dt=1, isAdapt=1):
    ...
    dv = 0.04*mem**2+5*mem+0.14-b+inputs
    mem = mem + dv
    ...
    inputs_ = mem - 0.3
    spike = act_fun_adp(inputs_)  # act_fun : approximation firing function
    mem = (1-spike)*mem + spike*c
    b = b+ spike*d
    return mem, spike, db, b
...
#     dv = 0.04*mem**2+5*mem+140-b+inputs
#     mem = mem + dv
...
class SNN_rec_cell(nn.Module):
    def __init__(self, input_size, hidden_size,is_rec = True):
        ...
        self.layer1_tauAdp = nn.Parameter(torch.Tensor(hidden_size))#nn.Linear(2*hidden_size, hidden_size)
        self.layer1_tauM = nn.Parameter(torch.Tensor(hidden_size))#nn.Linear(2*hidden_size, hidden_size)

        self.act1 = nn.Sigmoid()


        nn.init.xavier_uniform_(self.layer1_x.weight)
        # nn.init.normal_(self.layer1_tauAdp, 4.6,1)
        # nn.init.normal_(self.layer1_tauM, 3,1)

        nn.init.normal_(self.layer1_tauAdp, 0,1)
        nn.init.normal_(self.layer1_tauM, 0,1)

        # nn.init.uniform_(self.layer1_tauAdp, -1,1)
        # nn.init.uniform_(self.layer1_tauM, -1.,1)

        # nn.init.normal_(self.layer1_tauAdp, 200,50)
        # nn.init.normal_(self.layer1_tauM, 20,5)
    ...
    def forward(self, x_t, mem_t,spk_t,b_t):    
        ...
        # tauM1 = torch.exp(-1./self.layer1_tauM)
        # tauAdp1 = torch.exp(-1./self.layer1_tauAdp)
        
        tauM1 = self.act1(self.layer1_tauM)
        tauAdp1 = self.act1(self.layer1_tauAdp)
...
class SNN(nn.Module):
    def __init__(self, input_size, hidden_size,output_size, n_timesteps, P=10):
        ...
        self.snn3 = SNN_rec_cell(hidden_size*2,hidden_size)
