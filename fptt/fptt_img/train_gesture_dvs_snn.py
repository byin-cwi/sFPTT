...
# python train_gesture_dvs_snn.py --dataset DVS-Gesture --parts 28 --batch_size 16 --nhid 512 --lr 2e-3 --when 20 80 120 --epochs 8 --optim Adamax --log-interval 2
...
# rnn model
from snn_models_LIF4_dvs_gesture_dp import * 
from snn_models_LIF4_dvs_gesture_dp_asrnn import * 
from snn_models_LIF4_dvs_gesture_dp_izh import * 
# from snn_models_LIF4_dvs_gesture_dp2 import *
#python train_gesture_dvs_snn.py --dataset DVS-Gesture --parts 28 --batch_size 128 --nhid 512 --lr 3e-3 --when 80 120 --epochs 50 --optim Adam --log-interval 2
# t = 1000
# python train_gesture_dvs_snn.py --dataset DVS-Gesture --parts 28 --batch_size 32 --nhid 512 --lr 3e-3 --when 20 50 --epochs 100 --optim Adamax --log-interval 8
...
def test(model, test_loader, logger):
    ...
    for i ,(data, target) in enumerate(test_loader):
        ...
        data = data.view(-1, seq_length,2,128,128)#.permute(0,1,2,4,3)
        with torch.no_grad():
            model.eval()
            ...
            output = outputs[-1]
            # output = torch.stack(outputs[-5:]).mean(dim=0)
...
def train(epoch, args, train_loader, permute, n_classes, model, named_params, logger):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        for p in range(_PARTS):
            if p==0:
                h = model.init_hidden(data.size(0)) # initialize the hidden states at first timestep
            else:
                h = tuple(v.detach() for v in h) # detach the gradient from previous timestep
            ...
            fr_mean.append(model.network.fr)
            ...
            clf_loss = (p+1)/(_PARTS)*F.nll_loss(output, target) # rnn
            # clf_loss = F.cross_entropy(output, target) # cnn
            # clf_loss = (p+1)/(_PARTS)*F.cross_entropy(output, target) # cnn
        ...
        if batch_idx > 0 and batch_idx % args.log_interval == 0:
            logger.info('Train Epoch: {} [{}/{} ({:.0f}%)]\tlr: {:.6f}\tLoss: {:.6f}\tOracle: \
                {:.6f}\tClf: {:.6f}\tReg: {:.6f}\tFr: {:.6f}\tSteps: {}'.format(
                   ...
                   total_clf_loss / args.log_interval, total_regularizaton_loss / args.log_interval, np.mean(fr_mean),steps))
...
steps = 0
if args.dataset in ['CIFAR-10', 'MNIST-10','CIFAR-DVS','DVS-Gesture']:
...
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=30)# cnn
...
for epoch in range(1, epochs + 1):
    ...
    if args.dataset in ['CIFAR-10', 'MNIST-10','CIFAR-DVS','DVS-Gesture']:
        ...
        # test_loss, acc1 = test( model, train_loader, logger )
