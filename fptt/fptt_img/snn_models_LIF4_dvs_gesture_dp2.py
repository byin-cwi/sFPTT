...
class SNN(nn.Module):
    def __init__(self, input_size, hidden_size,output_size, n_timesteps, P=10):
        ...
        self.snn3 = SNN_rec_cell(hidden_size*2,hidden_size,False)
        self.snn4 = SNN_rec_cell(hidden_size,hidden_size)
    ...
    def forward(self, inputs, h):
        ...
        mem_4,spk_4,b_4 = self.snn4(spk_3, mem_t=h[9],spk_t=h[10],b_t = h[11])
        spk_4_dp = self.dp3(spk_4)

        dense3_x = self.layer3_x(spk_4_dp)
        ...
        self.fr = self.fr+ spk_1.detach().cpu().numpy().mean()/4.\
                + spk_2.detach().cpu().numpy().mean()/4\
                + spk_3.detach().cpu().numpy().mean()/4.\
                + spk_4.detach().cpu().numpy().mean()/4.
