...
class SNN(nn.Module):
    def __init__(self, input_size, hidden_size,output_size,  P=10):
        ...
        nn.init.normal_(self.yolo_tau1, 1.,1)
        self.act = nn.Sigmoid()
        ...
        nn.init.normal_(self.yolo_tau2, 1.,1)
    ...
    def forward(self, inputs, h,cuda=True):
        self.fr = 0
        h = list(h)
        ...
        x1 = predict_transform(h[-2].data, w, self.anchors[0], self.num_classes, cuda) # 75,13,13
        x2 = predict_transform(h[-1].data, w, self.anchors[1], self.num_classes, cuda) # 75,26,26
        # print(x1.shape, x2.shape)
        detections = torch.cat((x1, x2), 1)
        ...
        return [detections, h[-1],h[-2]], final_state, _

class SeqModel(nn.Module):
    def __init__(self, ninp, nhid, nout, parts=10):
        ...
        self.network = SNN(input_size=ninp, hidden_size=nhid, output_size=nout, P=parts)
        self.layer_size = self.network.network_size
        print(self.layer_size)
        
        self.l2_loss = nn.CrossEntropyLoss()

    def forward(self, inputs, hidden, CUDA=True):
        outputs = []
        for i in range(self.p):
            output, hidden, hiddens= self.network.forward(inputs, hidden,CUDA)
